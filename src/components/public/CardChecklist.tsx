...
export default function CardChecklist({ cards, displayMode }: Props) {
  ...
  const visibleCards =
    displayMode === "missing_only"
      ? cards
          .map((c) => ({
            ...c,
            variants: c.variants?.filter((v) => !v.isOwned) ?? [],
          }))
  ...
  if (cards.length === 0) {
    return (
      <p className="text-center text-[#8e8e93] py-12">
        Checklist zatím není k dispozici.
      </p>
    )
  }
    ...
    <div className="space-y-4">
      ...
      {displayMode === "missing_only" && (
        <p className="text-sm text-[#8e8e93]">
          Zobrazeno {filtered.length} karet, které ještě chybí.
        </p>
      )}
