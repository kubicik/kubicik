...
- scoreSpurs a scoreOpponent jsou celá čísla — výsledek na konci utkání (ne po prodloužení pokud nechceš)
...
export default function MatchImportExportButton() {
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (!res.ok) {
        setError(resp.error || "Chyba při importu")
        return
      }
      setResult({ imported: resp.imported })
      setJson("")
      router.refresh()
    } catch (err) {
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-16 overflow-y-auto">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#e5e5ea]">
              <h2 className="font-semibold text-[#1d1d1f]">Import zápasů z JSON</h2>
              <button
                onClick={handleClose}
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="bg-[#f0f6ff] rounded-xl p-4 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-medium text-[#007aff]">Vygenerovat zápasy pomocí AI</p>
                </div>
                <p className="text-xs text-[#3a3a3c]">
                  Zkopírujte prompt, předejte ho Claude, ChatGPT nebo jiné AI spolu s informacemi o zápasech. Výsledný JSON array vložte níže.
                </p>
                <button
                  onClick={copyPrompt}
              ...
              {/* Success state */}
              {result && (
                <div className="bg-[#f0fff4] border border-[#b7ebc8] text-[#1a7f37] text-sm px-4 py-3 rounded-xl flex items-center gap-2">
                  <svg className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                  Importováno {result.imported} {result.imported === 1 ? "zápas" : result.imported < 5 ? "zápasy" : "zápasů"}.
                </div>
              )}
              ...
              <div className="flex items-center justify-end gap-3 pt-1">
                ...
                >
                  {loading ? "Importuji…" : "Importovat zápasy"}
                </button>
              </div>
            </div>
          </div>
