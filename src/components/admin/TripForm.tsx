  ...
  return (
    <div className="relative">
      ...
      {open && filtered.length > 0 && (
        <ul className="absolute z-20 top-full mt-1 left-0 right-0 bg-white border border-[#e5e5ea] rounded-xl shadow-lg overflow-hidden max-h-48 overflow-y-auto">
          {filtered.map((name, i) => (
            <li key={name}>
              <button
                type="button"
                ...
                className={`w-full text-left px-3 py-2 text-sm transition-colors ${
                  i === highlighted ? "bg-[#007aff] text-white" : "hover:bg-[#f2f2f7] text-[#1d1d1f]"
                }`}
              >
                {name}
              </button>
...
export default function TripForm({ initial }: Props) {
  ...
  useEffect(() => {
    fetch("/api/attendees").then((r) => r.json()).then(setParticipantSuggestions).catch(() => {})
  }, [])
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-[0_2px_10px_rgba(0,0,0,0.06)] p-6 space-y-4">
        ...
        <div>
          <ParticipantChips value={participants} onChange={setParticipants} suggestions={participantSuggestions} />
